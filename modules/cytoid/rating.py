...
async def get_rating(uid, query_type):
    try:
        ...
        if 'statusCode' in Profile_json and Profile_json['statusCode'] == 404:
        ...
        traceback.print_exc()
        return {'status': False, 'text': f'发生错误：{str(e)}'}


async def download_cover_thumb(uid):
    try:
        d = abspath(f'./assets/cytoid-cover/{uid}/')
        if not os.path.exists(d):
            os.makedirs(d)
        path = f'{d}/thumbnail.png'
        if not os.path.exists(d):
            os.mkdir(d)
...
async def make_songcard(coverpath, chart_type, difficulty, chart_name, score, acc, rt, playtime, rank, details,
    ...
    Logger.debug(f'Image generated: {str(rank)}')
    return {int(rank): img}
