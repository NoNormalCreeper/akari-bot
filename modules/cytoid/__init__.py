...
async def _(msg: MessageSession):
    ...
    if query:
        ...
        if c == 0:
            ...
            if msg.target.targetFrom != 'TEST|Console' and img['status']:
                qc.reset()
        else:
...
async def _(msg: MessageSession):
    ...
    if getcode:
        if bind := CytoidBindInfoManager(msg).set_bind_info(
            username=getcode[0]
        ):
            m = f'{getcode[1]}({getcode[0]})' if getcode[1] else getcode[0]
...
async def _(msg: MessageSession):
    if unbind := CytoidBindInfoManager(msg).remove_bind_info():
