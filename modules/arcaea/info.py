...
async def get_info(usercode):
    ...
    try:
        get_ = await get_url(
            f"{api_url}user/info?usercode={usercode}&recent=1&withsonginfo=True",
            status_code=200,
            headers=headers,
            fmt='json',
        )
    ...
    if get_["status"] != 0:
        return (
            Plain(f'查询失败：{errcode[get_["status"]]}')
            if get_['status'] in errcode
            else Plain(f'查询失败。{get_}')
        )
    ...
    usrptt = '--' if usrptt == -1 else usrptt / 100
